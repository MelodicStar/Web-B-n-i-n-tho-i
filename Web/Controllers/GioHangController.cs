    ...
    public class GioHangController : Controller
    {
        ...
        public ActionResult DatHang(FormCollection collection)
        {
            try
            {
                ...
                var paymentMethod = Convert.ToInt32(collection["TrangThai"]);
                if (paymentMethod == 3)
                {
                    var orderId = Guid.NewGuid().ToString();
                    var paymentUrl = _vnpayService.CreatePaymentUrl((decimal)TongTien(), "Thanh toán đơn hàng", orderId);
        ...
        public ActionResult VNPayReturn()
        {
            ...
            // Verify the secure hash and response code
            if (responseCode == "00")
            {
                // Payment successful
                // Create the order
                ...
                if (kh != null && gh.Any())
                {
                    DonHang dh = new DonHang
                    {
                        ...
                        NgayGiaoHangDuKien = DateTime.Now.AddDays(1), // Example delivery date
                        TrangThai = "chuagiao",
                        TrangThaiThanhToan = "dathanhtoan",
                        MaTT = 3, // VNPay payment method
                        TongTienDonHang = (decimal)TongTien(),
                        GhiChu = "", // Add any notes if needed
                        ChietKhau = 0,
                        PhiVanChuyen = 25000,
                        DiaChiGiao = kh.DiaChi,
                        ThueVAT = 10,
                        TienThueVAT = (decimal)TongTien() * 0.1m,
                        ThanhTienTruocVAT = (decimal)TongTien() + 25000,
                        ThanhTienSauVAT = (decimal)TongTien() + 25000 + ((decimal)TongTien() * 0.1m),
                        NgayTao = DateTime.Now
                    };
                    ...
                    // Clear the cart
                    var dbCart = data.GioHangs.FirstOrDefault(g => g.MaKH == kh.MaKH && g.TrangThai == true);
                    if (dbCart != null)
                    {
                        var chiTietGioHang = data.ChiTietGioHangs.Where(ct => ct.MaGioHang == dbCart.MaGioHang);
                        data.ChiTietGioHangs.DeleteAllOnSubmit(chiTietGioHang);
                        dbCart.TrangThai = false;
                    }
